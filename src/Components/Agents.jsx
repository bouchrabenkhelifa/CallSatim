...
import { FaUser } from 'react-icons/fa'; // Import profile icon from React Icons
...
function Agents() {
    ...
    const handleDelete = (id) => {
        const filteredUsers = users.filter(agent=> agent.id !== id);
        setUsers(filteredUsers);

    };

  return (
    <div className=' bg-[#E7E7E3] py-2  '>
    <div className='p-4 my-5 mx-auto flex w-[90%] bg-white rounded shadow-md  flex-col gap-4 overflow-y-auto'>
      ...
      <h2 className="text-lg font-bold">Liste des agents</h2>
      <a href="/Ajouter_agents">
      <button className='bg-[#FF0000] border border-black-100 shadow px-4 py-2 rounded-l text-white font-bold text-xs'>
        Ajouter Agent
      </button></a>
      </div>
      <hr className="mb-4" />
                ...
                <div>
                    {agents.map((agent, index) => (
                        <div key={index} className="flex px-4 items-center mb-2 border-b last:border-none py-3">
                            ...
                            <span className="pl-4 flex-1">{agent.nom}</span>
                            <span className="pl-6 flex-1">{agent.prenom}</span>
                            <span className="pl-6 flex-1">{agent.Date}</span>
                            <span className="pl-6 flex-1">{agent.telephone}</span>
                            <div className='flex gap-5'>
              <a to={`/edit/${agent.id}`}><img src={Pen} alt="Edit" className='w-4 mr-3 ' /></a>
